...
export const MiniCalendarDays = ({
  ...
  const { month } = formatDate(days.map((date) => date)[0]);

  return (
    <div className="w-full flex-col justify-center text-center gap-4">
      <Label className="w-full text-center justify-center mb-2 capitalize">{month}</Label>
      <div className={cn("grid grid-cols-2 items-center gap-2 w-full overflow-hidden", className)} {...props}>
...
export const MiniCalendarDay = ({
  ...
  return (
    <Button
      className={cn(
        "h-[24px] w-full flex-col gap-0 p-2 text-xs",
